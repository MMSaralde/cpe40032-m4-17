...
CMPE 40032
    ...
    simply display "CRUSH-3" in large text, as well as a message to press
    Enter to begin.
]]
...
--[[
    Draw the centered CRUSH-3 text with background rect, placed along the Y
    axis as needed, relative to the center.
]]
function StartState:drawCRUSH3Text(y)
    -- draw semi-transparent rect behind CRUSH 3
    love.graphics.setColor(255, 255, 255, 128)
    love.graphics.rectangle('fill', VIRTUAL_WIDTH / 2 - 76, VIRTUAL_HEIGHT / 2 + y - 11, 150, 58, 6)

    -- draw CRUSH 3 text shadows
    love.graphics.setFont(gFonts['large'])
    self:drawTextShadow('CRUSH 3', VIRTUAL_HEIGHT / 2 + y)

    -- print CRUSH 3 letters in their corresponding current colors
    for i = 1, 6 do
